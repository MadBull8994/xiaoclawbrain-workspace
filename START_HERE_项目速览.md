...
## 当前执行路线（2026-05-29 对齐版）

详细执行顺序以 [项目开发路线图_XiaoClawBrain.md](./项目开发路线图_XiaoClawBrain.md) 为准。当前第一版路线按 P0-P9 推进；P9 稳定验收后，如需继续增强交互体验，再进入 P10 候选升级：

| 阶段 | 名称 | 目标 |
|------|------|------|
| P0 | 资料、仓库与现状对齐 | 统一文档口径，确认当前从哪里继续 |
| P1 | 仓库基线与开发环境固定 | 后端恢复正式 Git/分支状态，ESP32 梳理未提交改动 |
| P2 | 后端协议骨架与鉴权硬化 | Token、帧大小限制、JSON/binary 边界、smoke test |
| P3 | 后端稳定听、稳定想、稳定说 | ASR/LLM/TTS timeout、错误协议、状态机、ASR_EMPTY |
| P4 | 后端持久化、日志与运维 | SQLite session 记忆、日志脱敏、Docker healthcheck |
| P5 | ESP32 瘦客户端协议适配 | 只做 WS、音频 I/O、TFT、按键和状态处理 |
| P6 | ESP32 旧架构清理与本地降级 | 移除旧 base64 TTS、本地 provider、bridge/mimiclaw 主链路依赖 |
| P7 | 端到端联调与异常恢复 | 连续对话、断线重连、空识别、超时、播放失败验证 |
| P8 | 自定义 Skill 与 Agent 循环 | 主链路稳定后加入可配置安全等级的自定义 Skill 和 Agent 调度 |
| P9 | ESP32 本地语音唤醒 | 产品闭环必做，不能用按键触发替代 |
| P10 | 后续升级候选：可选连续对话与记忆策略升级 | 默认单轮，后台可切连续对话，并补齐长连接下的记忆保存/查询策略 |

当前实际进度：

- P0-P4 后端基础线已完成并经过多轮测试。
- P5 ESP32 瘦客户端主链路已具备 BOOT 录音、binary 上传、后端处理、binary TTS 下发和设备播放能力。
- P6 ESP32 旧架构清理已完成主链路收口：旧 `tts_response.audio` base64 播放入口已禁用；本地 `baidu_* provider`、`bridge`、`mimi/context_builder` 不在当前 `main/CMakeLists.txt` 编译主链路中，历史目录仅保留作后续归档/参考。
- P7 端到端联调与异常恢复已完成收口验收；Baidu STREAM ASR/TTS、长回复、工具轮、播放中 BOOT abort、连续 5 轮实机复测和 fast path 第二版修补均已通过。
- 百度 ASR/TTS 已切换到运行配置，其中 ASR 已改为 `InterfaceType.STREAM`；相关稳定性复测与 fast path 收口已完成。
- P8 已完成 P8-1 Skill 发现与扫描、P8-2 SkillRunner high 模式、P8-3 内置样例 Skill、P8-3 验收/测试清理收口、P8-4 全局默认 Skill、P8-5 SkillMatcher 与收尾修补、P8-6 Agent Loop（`llm_tool_call` 型 Skill 接入 LLM tool/function 路径，复用现有 `chat()` 工具链）、**P8-7 远程归档导入到 pending**（本地目录导入、远程 archive URL 导入、symlink 安全检查、归档根目录即 skill 包兼容），以及 **P8-8 pending 启用流程**（`SkillEnabler`，pending→enabled 移动、SKILL.md/manifest 校验、security_level 限制、sidecar 元信息、回滚、42 条测试）。P8-8 已验收通过。
- P9 本地语音唤醒已完成多轮实机复测（10 轮全部通过，2026-06-02），覆盖短问题、稍长问题、连续相邻、BOOT 备用入口等场景，无 MQTT fallback、无状态异常、无首词丢失。**P9 稳定验收通过**。
- P10 候选升级项已记录：如后续需要，再增加“默认单轮、后台可选连续对话”的会话模式，并同步补齐短期/长期记忆在长连接场景下的保存与查询策略。

当前状态：**P9 稳定验收通过**（2026-06-02 多轮实机复测 10/10 全部通过），且 **P6 主链路清理已收口**。当前第一版 P0-P9 核心闭环全部完成。可选连续对话与记忆策略升级已记录为 P10 候选，是否启动待后续评估。
