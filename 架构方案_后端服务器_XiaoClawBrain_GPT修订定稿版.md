...
以下能力不进入 P0-P9 第一阶段主闭环，避免一次性复杂度过高：

```text
- 后端 Cron 调度
- 后台可选连续对话模式（默认单轮；第一阶段不做，转入第二阶段会话模式升级）
...
上述仍有价值的暂缓项，不再保留为零散 `P10` 或“以后再说”口径，而是统一转入第二阶段路线图：

```text
第二阶段 P1-P6：
- P1 配置与持久化基线
- P2 角色、声音与默认 Skill 自定义
- P3 会话模式与记忆策略升级
- P4 交互体验增强
- P5 系统与平台增强
- P6 后台 UI 与配置页对齐评估
...
详细拆分见 `第二阶段开发路线图_XiaoClawBrain.md`。
