...
创建时间：2026-05-17
最近更新：2026-06-04（第一阶段稳定点已收口并通过后端全量测试）

更新规则：每次完成任务后立即更新，最新条目在上方。每次更新需包含：标记完成项、更新当前状态、记录当前进度、明确下一步要做的任务。

路线图：
- [项目开发路线图_XiaoClawBrain.md](./项目开发路线图_XiaoClawBrain.md)（第一阶段 P0-P9 历史路线）
- [第二阶段开发路线图_XiaoClawBrain.md](./第二阶段开发路线图_XiaoClawBrain.md)（当前后续入口）

---

## 第一阶段稳定点已收口（2026-06-04）：文档尾巴清理完成，后端 329/329 测试通过

目标：把“第一阶段已完成”的文档结论真正整理成一个明确的提交/稳定点；如果测试仍暴露尾巴，就先补掉再收口。

### 完成项

- [x] **确认三仓当前边界**
  - 根目录工作区仓库：存在本轮文档整理改动
  - 后端仓库 `xiaozhi-esp32-server`：存在尚未提交的一阶段主线改动
  - 固件仓库 `xiaoclaw-ghproxycom`：当前工作树干净
...
- [x] **复跑后端第一阶段全量测试**
  ...
  - 首轮结果：
    - 329 个测试里 1 个错误
    - 失败用例：
      - `test_recursive_chat_sends_llm_timeout_protocol`

- [x] **补掉稳定点前最后一个真实尾巴**
  - 根因：
    - `enqueue_tool_report()` 直接读取 `ConnectionHandler.read_config_from_api`
    - 递归工具调用超时测试使用最小化 `ConnectionHandler.__new__()` 构造，未完整初始化该属性
  - 修复：
    - 在 `xiaozhi-esp32-server/main/xiaozhi-server/core/handle/reportHandle.py`
    - 将工具上报前置判断改为 `getattr(...)` 安全降级
...
- 第一阶段现在不只是文档口径完成，后端这批一阶段主线改动也已通过全量测试复核
- 这次额外补掉的尾巴属于“测试和异常路径鲁棒性收口”，不会改变既有一阶段主链路结论
- 当前已经具备整理成明确提交/稳定点的条件

### 下一步

- 在根目录工作区仓库提交第一阶段文档收口与第二阶段入口整理
- 在后端仓库提交一阶段主线改动与这次测试尾巴修补
- 提交后再看是否需要同步推送远端，作为新的明确稳定点

---

## 第一阶段收口复核完成，第二阶段入口已建立（2026-06-04）

目标：复核“第一阶段是否都完成、是否还有没收干净的尾巴”；若第一阶段结论仍成立，则把第二阶段启动入口整理成统一口径。

### 完成项

- [x] **复核第一阶段完成结论**
  - 交叉检查 `START_HERE_项目速览.md`
  - 交叉检查 `项目开发路线图_XiaoClawBrain.md`
  - 交叉检查 `项目开发进度_XiaoClawBrain.md`
  - 交叉检查 `CLAUDE.md`
  - 复核结论：
    - P0-P9 核心闭环完成的结论保持成立
    - P6 旧架构清理已收口
    - P9 本地语音唤醒 10/10 实机复测通过的验收结论保持成立
    - 本轮未发现需要重新打开第一阶段验收的新增功能性阻塞

- [x] **清理阶段切换残留文档尾巴**
  - 更新 `START_HERE_项目速览.md`
  - 更新 `项目开发路线图_XiaoClawBrain.md`
  - 更新 `CLAUDE.md`
  - 更新 `架构方案_后端服务器_XiaoClawBrain_GPT修订定稿版.md`
  - 清理内容：
    - 仍停留在 “P9 还在收尾” 的旧口径
    - 仍停留在 “下一步先做 P8 / 不提前启动 P9” 的旧快照
    - 仍把后续工作零散写成 `P10` 候选的旧入口

- [x] **建立第二阶段正式入口**
  - 新建 `第二阶段开发路线图_XiaoClawBrain.md`
  - 将后续工作统一整理为：
    - 第二阶段 P1：配置与持久化基线
...
- 第一阶段 P0-P9 维持“已完成”的结论，不需要因为本轮复核重新开阶段
- 本轮发现的“尾巴”主要是文档阶段切换没有同步干净，不是主链路缺功能
- 当前项目后续入口已从零散 `P10` 候选切换为明确的第二阶段路线

### 下一步

- 从 **第二阶段 P1：配置与持久化基线** 开始
- 先明确哪些默认配置必须跨更新保留
- 固定默认 global skill、默认百度克隆音色、默认 agent persona 的权威存储位置
